        ...
        if(*asmcmd == '.')                 // Force decimal number
        {
            ...
            if(isdigit(*asmcmd) || toupper(*asmcmd) == 'E')
            {
                ...
                if(toupper(*asmcmd) == 'E')
                {
                    ...
                    floating *= powl(10.0, decimal * base);
                }; //TODO: was "pow10l": http://refspecs.linuxbase.org/LSB_4.1.0/LSB-Core-generic/LSB-Core-generic/baselib-pow10l.html
                fdata = floating;
                scan = SCAN_FCONST;
                return;
            }
             ...
             int constsize, char* errtext)
{
    ...
    {
        if(errtext != NULL) strcpy_s(errtext, TEXTLEN, "Internal OLLYDBG error");
        return 0;
    };                       // Error in parameters
    asmcmd = cmd;
    rep = lock = 0;
    ...
    while(1)                             // Fetch all REPxx and LOCK prefixes
    {
        if(scan == SCAN_REP || scan == SCAN_REPE || scan == SCAN_REPNE)
        {
            if(rep != 0)
            {
                strcpy_s(errtext, TEXTLEN, "Duplicated REP prefix");
                goto error;
            };
            rep = scan;
        }
        else if(scan == SCAN_LOCK)
        {
            if(lock != 0)
            {
                strcpy_s(errtext, TEXTLEN, "Duplicated LOCK prefix");
                goto error;
            };
            lock = scan;
        }
    ...
    {
        strcpy_s(errtext, TEXTLEN, "Command mnemonic expected");
        goto error;
    };
    nameend = asmcmd;
    _strupr_s(sdata, TEXTLEN);
    ...
    if(scan == SCAN_ERR)
    {
        strcpy_s(errtext, TEXTLEN, asmerror);
        goto error;
    };
    if(scan != SCAN_EOL)
    {
        strcpy_s(errtext, TEXTLEN, "Extra input after operand");
        goto error;
    };
...
retrylongjump:
    ...
    for(pd = cmddata; pd->mask != 0; pd++)
    {
        ...
        for(j = 0; j < 3; j++)             // Up to 3 operands
        {
            ...
            default:                       // Undefined type of operand
                strcpy_s(errtext, TEXTLEN, "Internal Assembler error");
                goto error;
            };                               // End of switch (arg)
    ...
    {
        strcpy_s(errtext, TEXTLEN, "Unrecognized command");
        asmcmd = nameend;
        goto error;
    };
    if(match != 0)                       // Command not found
    {
        if(minop > 0 && aop[minop - 1].type == NNN)
            strcpy_s(errtext, TEXTLEN, "Too few operands");
        else if(maxop < 3 && aop[maxop].type != NNN)
            strcpy_s(errtext, TEXTLEN, "Too many operands");
        else if(nameok > 1)                // More that 1 command
            strcpy_s(errtext, TEXTLEN, "Command does not support given operands");
        else if(match & MA_JMP)
            strcpy_s(errtext, TEXTLEN, "Invalid jump size modifier");
        else if(match & MA_NOP)
            strcpy_s(errtext, TEXTLEN, "Wrong number of operands");
        else if(match & MA_TYP)
            strcpy_s(errtext, TEXTLEN, "Command does not support given operands");
        else if(match & MA_NOS)
            strcpy_s(errtext, TEXTLEN, "Please specify operand size");
        else if(match & MA_SIZ)
            strcpy_s(errtext, TEXTLEN, "Bad operand size");
        else if(match & MA_DIF)
            strcpy_s(errtext, TEXTLEN, "Different size of operands");
        else if(match & MA_SEG)
            strcpy_s(errtext, TEXTLEN, "Invalid segment register");
        else if(match & MA_RNG)
            strcpy_s(errtext, TEXTLEN, "Constant out of expected range");
        else
            strcpy_s(errtext, TEXTLEN, "Erroneous command");
        goto error;
    };
    ...
    *(ulong*)tmask = pd->mask;
    ...
    for(j = 0; j < 3; j++)               // Up to 3 operands
    {
        ...
        case MFX:                        // Memory in ModRM byte (ext. FPU state)
            ...
            else                           // SIB byte necessary
            {
                ...
                {
                    strcpy_s(errtext, TEXTLEN, "Invalid indexing mode");
                    goto error;
                };
                if(op->base < 0)
                {
        ...
        case SCM:                        // Segment register in command byte
            ...
            else if(op->index < 6)
            {
                ...
                {
                    strcpy_s(errtext, TEXTLEN, "Unable to POP CS");
                    goto error;
                };
    ...
    if(segment != SEG_UNDEF)             // Segment prefix necessary
    {
        ...
        else { strcpy_s(errtext, TEXTLEN, "Internal Assembler error"); goto error; };
        model->mask[j] = 0xFF;
        j++;
    };
    if(dispsize > 0)
    ...
    if(immsize > 0)
    {
        ...
        {
            strcpy_s(errtext, TEXTLEN, "Constant does not fit into operand");
            goto error;
        };
