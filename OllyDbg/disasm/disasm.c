...
static void DecodeRG(int index, int datasize, int type)
{
    ...
    if(mode >= DISASM_FILE)
    {
        ...
        if(lowercase) _strlwr_s(name, 9);
        if(type < PSEUDOOP)                // Not a pseudooperand
            nresult += sprintf_s(da->result + nresult, TEXTLEN - nresult, "%s", name);
        ;
    };
};
...
static void DecodeST(int index, int pseudoop)
{
    ...
    if(pseudoop == 0)
    {
        strcpy_s(da->result + nresult, TEXTLEN - nresult, s);
        nresult += i;
    };
};
...
static void DecodeMX(int index)
{
    ...
    nresult += sprintf_s(pr, TEXTLEN - nresult, "%s%i", (lowercase ? "mm" : "MM"), index);
};
...
static void Memadr(int defseg, const char* descr, long offset, int dsize)
{
    ...
    pr = da->result + nresult;
    n = 0;
    if(segprefix != SEG_UNDEF) seg = segprefix;
    else seg = defseg;
    if(ideal != 0) pr[n++] = '[';
      ...
      )
    {
        if(dsize < sizeof(sizename) / sizeof(sizename[0]))
            n += sprintf_s(pr + n, TEXTLEN - nresult - n, "%s %s", sizename[dsize], (ideal == 0 ? "PTR " : ""));
        else
            n += sprintf_s(pr + n, TEXTLEN - nresult - n, "(%i-BYTE) %s", dsize, (ideal == 0 ? "PTR " : ""));
        ;
    };
    if((putdefseg != 0 || seg != defseg) && seg != SEG_UNDEF)
        n += sprintf_s(pr + n, TEXTLEN - nresult - n, "%s:", segname[seg]);
    if(ideal == 0) pr[n++] = '[';
    n += sprintf_s(pr + n, TEXTLEN - nresult - n, "%s", descr);
    if(lowercase) _strlwr_s(pr, TEXTLEN - nresult);
    if(offset == 0L)
    {
        if(*descr == '\0') pr[n++] = '0';
    }
    else
    {
        ...
        {
            if(*descr != '\0') pr[n++] = '+';
            strcpy_s(pr + n, TEXTLEN - nresult - n, s);
            n += i;
        }
        else if(offset < 0 && offset > -16384 && *descr != '\0')
            n += sprintf_s(pr + n, TEXTLEN - nresult - n, "-%lX", -offset);
        else
        {
            if(*descr != '\0') pr[n++] = '+';
            n += sprintf_s(pr + n, TEXTLEN - nresult - n, "%lX", offset);
    ...
    if(mode >= DISASM_DATA)
    {
        ...
        case MRJ:                        // Memory/reg in ModRM as JUMP target
            ...
            if(mode >= DISASM_FILE && shownear != 0)
                nresult += sprintf_s(da->result + nresult, TEXTLEN - nresult, "%s ", (lowercase ? "near" : "NEAR"));
            break;
        ...
        case MMS:                        // Memory in ModRM byte (as SEG:OFFS)
            ...
            if(datasize == 4 && inmemory)
                da->memtype = DEC_FWORD;
            if(mode >= DISASM_FILE)
                nresult += sprintf_s(da->result + nresult, TEXTLEN - nresult, "%s ", (lowercase ? "far" : "FAR"));
            break;
    ...
    else if((c & 0x07) == 0x04)          // SIB addresation
    {
        ...
        *s = '\0';
        ...
        else                               // Base and, eventually, displacement
        {
            ...
            if(mode >= DISASM_FILE)
            {
                strcpy_s(s, TEXTLEN, regname[2][j]);
                seg = addr32[j].defseg;
            };
        };
        ...
        if(mode >= DISASM_FILE && da->error == DAE_NOERR)
        {
            if((sib & 0x38) != 0x20)         // Scaled index present
            {
                if(*s != '\0') strcat_s(s, TEXTLEN, "+");
                strcat_s(s, TEXTLEN, addr32[(sib >> 3) & 0x07].descr);
                if((sib & 0xC0) == 0x40)
                {
                    da->jmptable = 0;            // Hardly a switch!
                    strcat_s(s, TEXTLEN, "*2");
                }
                else if((sib & 0xC0) == 0x80)
                    strcat_s(s, TEXTLEN, "*4");
                else if((sib & 0xC0) == 0xC0)
                {
                    da->jmptable = 0;            // Hardly a switch!
                    strcat_s(s, TEXTLEN, "*8");
                };
            };
...
static void DecodeIM(int constsize, int sxt, int type)
{
    ...
    if(mode >= DISASM_FILE && da->error == DAE_NOERR)
    {
        ...
        if(i != 0 && symbolic != 0)
        {
            strcpy_s(da->result + nresult, TEXTLEN - nresult, name);
            nresult += i;
        }
        else if(type == IMU || type == IMS || type == IM2 || data >= 0 || data < NEGLIMIT)
            nresult += sprintf_s(da->result + nresult, TEXTLEN - nresult, "%lX", data);
        else
            nresult += sprintf_s(da->result + nresult, TEXTLEN - nresult, "-%lX", -data);
        if(addcomment && comment[0] != '\0') strcpy_s(da->comment, TEXTLEN, comment);
    };
};
...
static void DecodeC1(void)
{
    if(mode < DISASM_DATA) return;
    da->immconst = 1;
    if(mode >= DISASM_FILE) nresult += sprintf_s(da->result + nresult, TEXTLEN - nresult, "1");
};
...
static void DecodeRJ(ulong offsize, ulong nextip)
{
    ...
    if(mode >= DISASM_FILE)
    {
        if(offsize == 1) nresult += sprintf_s(da->result + nresult, TEXTLEN - nresult,
        ...
        if(mode >= DISASM_CODE)
            ...
            i = 0;
        if(symbolic == 0 || i == 0)
            nresult += sprintf_s(da->result + nresult, TEXTLEN - nresult, "%08lX", addr);
        else
            nresult += sprintf_s(da->result + nresult, TEXTLEN - nresult, "%.*s", TEXTLEN - nresult - 25, s);
        if(symbolic == 0 && i != 0 && da->comment[0] == '\0')
            strcpy_s(da->comment, TEXTLEN, s);
        ;
    };
};
...
static void DecodeJF(void)
{
    ...
    if(mode >= DISASM_FILE)
    {
        nresult += sprintf_s(da->result + nresult, TEXTLEN - nresult, "%s %04X:%08X",
...
static void DecodeSG(int index)
{
    ...
    if(mode >= DISASM_FILE)
    {
        i = sprintf_s(da->result + nresult, TEXTLEN - nresult, "%s", segname[index]);
        if(lowercase) _strlwr_s(da->result + nresult, TEXTLEN - nresult);
        nresult += i;
    };
};
...
static void DecodeCR(int index)
{
    ...
    {
        index = (index >> 3) & 0x07;
        nresult += sprintf_s(da->result + nresult, TEXTLEN - nresult, "%s", crname[index]);
        if(lowercase) _strlwr_s(da->result + nresult, TEXTLEN - nresult);
    };
};
...
static void DecodeDR(int index)
{
    ...
    {
        index = (index >> 3) & 0x07;
        i = sprintf_s(da->result + nresult, TEXTLEN - nresult, "%s", drname[index]);
        if(lowercase) _strlwr_s(da->result + nresult, TEXTLEN - nresult);
        nresult += i;
    };
};
             ...
             t_disasm* disasm, int disasmmode)
{
    ...
    softerror = 0;
    is3dnow = 0;
    static t_disasm g_disasm;
    da = disasm = disasm ? disasm : &g_disasm;
    da->ip = srcip;
    da->comment[0] = '\0';
    da->cmdtype = C_BAD;
    da->nprefix = 0;
    ...
    while(size > 0)
    {
        ...
        {
        case 0x26:
            if(segprefix == SEG_UNDEF) segprefix = SEG_ES;
            else repeated = 1;
            break;
        case 0x2E:
            if(segprefix == SEG_UNDEF) segprefix = SEG_CS;
            else repeated = 1;
            break;
        case 0x36:
            if(segprefix == SEG_UNDEF) segprefix = SEG_SS;
            else repeated = 1;
            break;
        case 0x3E:
            if(segprefix == SEG_UNDEF) segprefix = SEG_DS;
            else repeated = 1;
            break;
        case 0x64:
            if(segprefix == SEG_UNDEF) segprefix = SEG_FS;
            else repeated = 1;
            break;
        case 0x65:
            if(segprefix == SEG_UNDEF) segprefix = SEG_GS;
            else repeated = 1;
            break;
        case 0x66:
        ...
        if(mode >= DISASM_FILE)
            ndump += sprintf_s(da->dump + ndump, TEXTLEN - ndump, "%02X:", *cmd);
        da->nprefix++;
        cmd++;
        srcip++;
        size--;
    ...
    if(repeated)
    {
        if(mode >= DISASM_FILE)
        {
            ...
            {
            case 0x26:
                pname = (char*)(segname[SEG_ES]);
                break;
            case 0x2E:
                pname = (char*)(segname[SEG_CS]);
                break;
            case 0x36:
                pname = (char*)(segname[SEG_SS]);
                break;
            case 0x3E:
                pname = (char*)(segname[SEG_DS]);
                break;
            case 0x64:
                pname = (char*)(segname[SEG_FS]);
                break;
            case 0x65:
                pname = (char*)(segname[SEG_GS]);
                break;
            case 0x66:
                pname = "DATASIZE";
                break;
            ...
            nresult += sprintf_s(da->result + nresult, TEXTLEN - nresult, "PREFIX %s:", pname);
            if(lowercase) _strlwr_s(da->result, TEXTLEN);
            if(extraprefix == 0) strcpy_s(da->comment, TEXTLEN, "Superfluous prefix");
        };
    ...
    if(lockprefix != 0)
    {
        if(mode >= DISASM_FILE) nresult += sprintf_s(da->result + nresult, TEXTLEN - nresult, "LOCK ");
    ...
    else                                 // Command recognized, decode it
    {
        ...
        for(operand = 0; operand < 3; operand++)
        {
            ...
            case JOW:                      // Immediate full offset (for jumps)
                DecodeRJ(datasize, srcip + datasize + 1);
                break;
    ...
    {
        if(mode >= DISASM_FILE)
            nresult = sprintf_s(da->result, TEXTLEN, "???");
        if(da->error == DAE_BADCMD &&
                (*cmd == 0x0F || *cmd == 0xFF) && size > 0
          )
        {
            if(mode >= DISASM_FILE) ndump += sprintf_s(da->dump + ndump, TEXTLEN - ndump, "%02X", *cmd);
            cmd++;
            size--;
        };
        if(size > 0)
    ...
    else                                 // No hard error, dump command
    {
        ...
        {
            ndump += sprintf_s(da->dump + ndump, TEXTLEN - ndump, "%02X", *cmd++);
            if(hasrm) ndump += sprintf_s(da->dump + ndump, TEXTLEN - ndump, "%02X", *cmd++);
            if(hassib) ndump += sprintf_s(da->dump + ndump, TEXTLEN - ndump, "%02X", *cmd++);
            if(dispsize != 0)
            {
                da->dump[ndump++] = ' ';
    ...
    if(mode >= DISASM_FILE)
    {
        if(da->error != DAE_NOERR) switch(da->error)
            {
            case DAE_CROSS:
                strcpy_s(da->comment, TEXTLEN, "Command crosses end of memory block");
                break;
            case DAE_BADCMD:
                strcpy_s(da->comment, TEXTLEN, "Unknown command");
                break;
            case DAE_BADSEG:
                strcpy_s(da->comment, TEXTLEN, "Undefined segment register");
                break;
            case DAE_MEMORY:
                strcpy_s(da->comment, TEXTLEN, "Illegal use of register");
                break;
            case DAE_REGISTER:
                strcpy_s(da->comment, TEXTLEN, "Memory address not allowed");
                break;
            case DAE_INTERN:
                strcpy_s(da->comment, TEXTLEN, "Internal OLLYDBG error");
                break;
            default:
                strcpy_s(da->comment, TEXTLEN, "Unknown error");
                break;
            }
        else if((da->warnings & DAW_PRIV) != 0 && privileged == 0)
            strcpy_s(da->comment, TEXTLEN, "Privileged command");
        else if((da->warnings & DAW_IO) != 0 && iocommand == 0)
            strcpy_s(da->comment, TEXTLEN, "I/O command");
        else if((da->warnings & DAW_FARADDR) != 0 && farcalls == 0)
        {
            if((da->cmdtype & C_TYPEMASK) == C_JMP)
                strcpy_s(da->comment, TEXTLEN, "Far jump");
            else if((da->cmdtype & C_TYPEMASK) == C_CAL)
                strcpy_s(da->comment, TEXTLEN, "Far call");
            else if((da->cmdtype & C_TYPEMASK) == C_RET)
                strcpy_s(da->comment, TEXTLEN, "Far return");
            ;
        }
        else if((da->warnings & DAW_SEGMENT) != 0 && farcalls == 0)
            strcpy_s(da->comment, TEXTLEN, "Modification of segment register");
        else if((da->warnings & DAW_SHIFT) != 0 && badshift == 0)
            strcpy_s(da->comment, TEXTLEN, "Shift constant out of range 1..31");
        else if((da->warnings & DAW_PREFIX) != 0 && extraprefix == 0)
            strcpy_s(da->comment, TEXTLEN, "Superfluous prefix");
        else if((da->warnings & DAW_LOCK) != 0 && lockedbus == 0)
            strcpy_s(da->comment, TEXTLEN, "LOCK prefix");
        else if((da->warnings & DAW_STACK) != 0 && stackalign == 0)
            strcpy_s(da->comment, TEXTLEN, "Unaligned stack operation");
        ;
    };
